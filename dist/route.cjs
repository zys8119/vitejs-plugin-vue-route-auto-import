    ...
    if (!ROUTES_FILTER_REG.test(key)) {
      ...
      for (let j = 0; j < chain.length; j++) {
        ...
        if (lastNode == currentNode) {
          ...
          if (directory) {
            const layoutPath = input.find(
              (e) => new RegExp(`^${path}/layout\\.(vue|jsx|tsx)`, "img").test(e)
            );
            if (layoutPath) {
              layoutComponent = files[layoutPath];
            }
          }
          const pageJsonPath = directory ? `${path}/page.json` : path.replace(
            new RegExp(`${chain[j]}\\.${suffix}$`, "i"),
            "page.json"
          );
          const directoryMeta = (directory ? pages[pageJsonPath] : pages[pageJsonPath.replace(
            new RegExp(`(${wantedNode}$)`),
            "page.json"
          )]) || {};
          const meta = directory ? directoryMeta[chain[j]] || {} : Object.assign(
            metaMaps[path] || {},
            (pages[pageJsonPath] || {})[`${chain[j]}.${suffix}`] || {}
          );
...
routes = routes.filter((e) => !e.layout).concat(
  pathToTree(
    routes.filter((e) => e.layout).map((e) => e.key),
    /\.(vue|jsx|tsx)$/
  )
);
